...
DigoleSerialDisp LCD(&Serial2, 9600);

const int LCDdelay=2;  // 10 is conservative, 2 actually works
...
void menuSetup()
{
  ...
  menu.getRoot().add(menu_mediab1); 
    ...
    menu_mediab2.addAfter(menu_trans);
...
void menuChangeEvent(MenuChangeEvent changed)
{
  ...
  if (changed.to.getName()=="MenuRoot")  {
    ...
    if (!(bool)changed.to.getRight())  {
        ...
        if (changed.to.getName() == menu_UP1.getName()) {
            lcdPosition(1,0);
...
void menuValues()  {
      ...
      if (setMenuValues != 0 && (encValue==1 || encValue==-1)) {
        timeoutMillis = millis() + 15000;
        timeoutDone = false;
      }
      ...
      switch(setMenuValues)  {
        ...
        case 2:  // Media bank 2 selector:
          ...
          menuValues_printValue(sVal+1,6,3, "Still");
        break;
        case 3:  // AUX 1
        case 4:  // AUX 2
        case 5:  // AUX 3
        ...
        case 11:  // Transition: Time
          ...
          //menuValues_clearValueLine();
          menuValues_printValue(sVal,8,3,"Frames:");
        break;
...
void menuValues_printValue(int number, uint8_t pos, uint8_t padding, String prefix)  {
          lcdPosition(1,0);
          if (prefix.length() > 0) LCD.print(prefix);
          for (int i = prefix.length();i<pos;i++) LCD.print(" ");
          LCD.print(number);
          for(int i=String(number).length()+pos; i<16; i++)  {
            LCD.print(" ");
          }
}
void menuValues_printSource(int sVal)  {
   switch(sVal)  {
       ...
       case 4010: LCD.print("Key Mask 1      "); break;
       case 5010: LCD.print("DSK1 Mask       "); break;
       case 5020: LCD.print("DSK2 Mask       "); break;
